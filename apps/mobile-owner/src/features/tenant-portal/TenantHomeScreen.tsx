...
export function TenantHomeScreen({ onNavigate }: TenantHomeScreenProps) {
  ...
  return (
    <Screen loading={loading} onRefresh={loadData} refreshing={loading} subtitle={data?.tenant ? `Welcome back, ${data.tenant.fullName}` : 'Welcome back'} title="Tenant Portal">
      {error ? <Text style={styles.error}>{error}</Text> : null}
      {!data?.tenant ? <Text style={styles.empty}>No tenant profile found. Please contact your boarding house owner.</Text> : null}
...
const styles = StyleSheet.create({
  ...
  notificationItem: { borderBottomColor: colors.border, borderBottomWidth: 1, gap: spacing.xs, paddingBottom: spacing.sm },
